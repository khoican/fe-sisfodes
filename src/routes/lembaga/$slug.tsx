        ...
        <main className="w-full">
                            ...
                            institution.mission.length > 0 && (
                                <div>
                                    ...
                                    <div className="flex flex-col gap-4">
                                        {institution.mission.map(
                                            (item, index) => (
                                                <div
                                                    key={index}
                                                    className="flex items-start gap-4 bg-white/10 p-4 rounded-xl border border-white/10"
                                                >
            ...
            {((institution.tasks && institution.tasks.length > 0) ||
                (institution.functions &&
                    institution.functions.length > 0)) && (
                <section className="px-4 lg:px-12 mt-16 mb-16">
                    <Title
                        title={
                            institution.tasks ? 'Tugas Pokok' : 'Fungsi Lembaga'
                        }
                    />

                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mt-8">
                        {(institution.tasks || institution.functions || []).map(
                            (item, index) => (
                                <div
                                    key={index}
                                    className="bg-card p-6 rounded-2xl border border-border shadow-sm flex gap-4 hover:border-primary/30 transition-colors"
                                >
            ...
            <section className="px-4 lg:px-12 mt-16 mb-20">
                ...
                <p className="text-muted-foreground mt-2 mb-10">
                    Daftar pengurus {institution.full_name} yang aktif saat ini.
                </p>
                ...
                {institution.members && institution.members.length > 0 ? (
                    ...
                    <div className="w-full py-12 bg-muted rounded-2xl border border-dashed border-border flex flex-col items-center justify-center text-muted-foreground/70">
                        <p>
                            Data kepengurusan belum tersedia atau sedang
                            diperbarui.
                        </p>
                    </div>
                )}
