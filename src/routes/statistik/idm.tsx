...
function IdmPage() {
  ...
  return (
    <main className='w-full'>
      {/* Hero Header */}
      <section className='bg-background px-4 lg:px-12 py-12 rounded-b-3xl shadow-sm border-b'>
        <div className='flex flex-col lg:flex-row gap-12 items-center'>
          <div className='flex-1'>
            ...
            <div className='flex flex-wrap gap-4 mt-8'>
              ...
              <div className='px-6 py-4 rounded-2xl border bg-muted flex flex-col gap-1'>
                <span className='text-[10px] font-bold uppercase tracking-wider text-muted-foreground/70'>Skor IDM {summary.TAHUN}</span>
                <span className='text-2xl font-black text-foreground'>{Number(summary.SKOR_SAAT_INI).toFixed(4)}</span>
              </div>
      ...
      <section className='px-4 lg:px-12 py-16 grid lg:grid-cols-2 gap-12'>
        <div>
          ...
          <p className='text-sm text-muted-foreground mb-8'>Perbandingan nilai antara dimensi ketahanan sosial, ekonomi, dan lingkungan.</p>
          
          <div className='h-[350px] w-full'>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={dimensionData} layout="vertical" margin={{ left: 40, right: 40 }}>
                <CartesianGrid strokeDasharray="3 3" horizontal={true} vertical={false} opacity={0.3} stroke='hsl(var(--border))' />
          ...
          <div className='grid grid-cols-3 gap-4 mt-8'>
             {dimensionData.map((dim) => (
               <div key={dim.name} className='p-4 bg-card rounded-2xl border text-center border-border'>
                  <dim.icon className='w-6 h-6 mx-auto mb-2' style={{ color: dim.color }} />
                  <p className='text-[10px] font-bold text-muted-foreground/70 uppercase'>{dim.name.split(' ')[0]}</p>
                  <p className='text-xl font-black text-foreground'>{dim.score.toFixed(4)}</p>
               </div>
             ))}
          </div>
        </div>
        ...
        <div>
           ...
           <div className='space-y-6 mt-8'>
              <Card className='border-none shadow-sm bg-blue-500/10'>
                 ...
                 <CardContent>
                    <p className='text-3xl font-black text-blue-600 dark:text-blue-400'>{summary.SKOR_MINIMAL}</p>
                    <p className='text-sm text-blue-600/70 dark:text-blue-400/70 mt-1'>Ambang batas untuk mempertahankan status Desa Mandiri.</p>
                 </CardContent>
              </Card>
              ...
              <Card className='border-none shadow-sm bg-green-500/10'>
                 ...
                 <CardContent>
                    <p className='text-3xl font-black text-green-600 dark:text-green-400'>{summary.PENAMBAHAN > 0 ? '+' : ''}{summary.PENAMBAHAN.toFixed(4)}</p>
                    <p className='text-sm text-green-600/70 dark:text-green-400/70 mt-1'>Perubahan skor dibandingkan dengan periode penilaian sebelumnya.</p>
                 </CardContent>
              </Card>

              <div className='bg-card p-6 rounded-2xl border border-dashed border-border flex items-start gap-4'>
                 <Info className='text-primary shrink-0' />
                 <div>
                    <h4 className='font-bold text-foreground'>Apa itu IDM?</h4>
      ...
      <section className='px-4 lg:px-12 py-16 bg-muted/50'>
         ...
         <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
            {indicatorRows.map((row, idx) => (
              <Card key={idx} className='border-none shadow-sm hover:shadow-md transition-shadow group overflow-hidden bg-card'>
                 ...
                 <CardHeader className='pb-2'>
                    <div className='flex justify-between items-start mb-2'>
                       ...
                       <div className='flex items-baseline gap-1'>
                          <span className='text-2xl font-black text-foreground'>{row.SKOR}</span>
                          <span className='text-[10px] text-muted-foreground/70 font-bold'>/ 5</span>
                       </div>
                    </div>
                    <CardTitle className='text-sm leading-tight text-muted-foreground'>{row.INDIKATOR}</CardTitle>
                 </CardHeader>
                 <CardContent>
                    <p className='text-xs text-muted-foreground/70 leading-relaxed line-clamp-2 italic'>"{row.KETERANGAN}"</p>
                    {row.KEGIATAN && row.KEGIATAN !== '-' && (
                       <div className='mt-4 pt-3 border-t border-border'>
                          <p className='text-[9px] font-bold text-primary uppercase mb-1'>Rekomendasi Kegiatan:</p>
                          <p className='text-[10px] text-muted-foreground'>{row.KEGIATAN}</p>
                       </div>
                    )}
                 </CardContent>
              </Card>
         ...
         {indicatorRows.length === 0 && (
           <div className='text-center py-20'>
              <p className='text-muted-foreground/70'>Tidak ada indikator yang cocok dengan pencarian Anda.</p>
           </div>
         )}
      </section>

      {/* Identity Footer */}
      <section className='px-4 lg:px-12 py-12 bg-background flex flex-col items-center text-center'>
         <div className='max-w-2xl'>
            ...
            <div className='mt-8 pt-8 border-t border-border w-full flex justify-center gap-12'>
               <div>
                  <p className='text-[10px] font-bold text-muted-foreground/70 uppercase'>Kode Desa</p>
                  <p className='text-sm font-bold'>{idm.IDENTITAS[0].id_desa}</p>
               </div>
               <div>
                  <p className='text-[10px] font-bold text-muted-foreground/70 uppercase'>Tahun Data</p>
