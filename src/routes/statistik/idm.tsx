    ...
    head: () => ({
        meta: [
            ...
            },
            {
                name: 'description',
                content:
                    'Data perkembangan kemajuan desa berdasarkan Indeks Desa Membangun (IDM) yang mencakup dimensi sosial, ekonomi, dan lingkungan.',
            },
        ],
    }),
...
function IdmPage() {
    ...
    const dimensionData = useMemo(() => {
        const iks =
            (rows.find((r) => r.INDIKATOR.startsWith('IKS'))?.SKOR as number) ||
            0
        const ike =
            (rows.find((r) => r.INDIKATOR.startsWith('IKE'))?.SKOR as number) ||
            0
        const ikl =
            (rows.find((r) => r.INDIKATOR.startsWith('IKL'))?.SKOR as number) ||
            0
    ...
    const indicatorRows = useMemo(() => {
        return rows
            .filter((r) => r.NO !== null && r.KETERANGAN !== null)
            .filter((r) =>
                r.INDIKATOR.toLowerCase().includes(searchQuery.toLowerCase()),
            )
    }, [rows, searchQuery])
                ...
                <div className="flex flex-col lg:flex-row gap-12 items-center">
                    <div className="flex-1">
                        ...
                        <h1 className="text-5xl font-extrabold leading-tight">
                            Indeks Desa{' '}
                            <span className="text-primary">Membangun</span>
                        </h1>
                        <p className="text-muted-foreground mt-6 text-lg max-w-2xl">
                            IDM adalah potret kemajuan kemandirian desa yang
                            diukur dari ketahanan sosial, ekonomi, dan
                            lingkungan. Desa kami terus berkomitmen meningkatkan
                            kualitas hidup warga.
                        </p>

                        <div className="flex flex-wrap gap-4 mt-8">
                            <div
                                className={`px-6 py-4 rounded-2xl border flex flex-col gap-1 ${getStatusColor(summary.STATUS)}`}
                            >
                            ...
                            <div className="px-6 py-4 rounded-2xl border bg-muted flex flex-col gap-1">
                                <span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground/70">
                                    Skor IDM {summary.TAHUN}
                                </span>
                                <span className="text-2xl font-black text-foreground">
                                    {Number(summary.SKOR_SAAT_INI).toFixed(4)}
                                </span>
                            </div>
                    ...
                    <div className="w-full lg:w-96 h-64 bg-primary/5 rounded-3xl p-8 flex flex-col items-center justify-center relative overflow-hidden">
                        <TrendingUp
                            className="absolute -bottom-10 -right-10 w-40 h-40 text-primary/10 -rotate-12"
                            aria-hidden="true"
                        />
                        <div className="relative z-10 text-center">
                            ...
                            <p className="text-sm text-muted-foreground mt-1 italic">
                                Berdasarkan Keputusan Menteri Desa
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <section className="px-4 lg:px-12 py-16 grid lg:grid-cols-2 gap-12">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground mb-8">
                        Perbandingan nilai antara dimensi ketahanan sosial,
                    ...
                    <div className="grid grid-cols-3 gap-4 mt-8">
                        {dimensionData.map((dim) => (
                            <div
                                key={dim.name}
                                className="p-4 bg-card rounded-2xl border text-center border-border"
                            >
                                <dim.icon
                            ...
                            <CardHeader className="pb-2">
                                <div className="flex items-center gap-3">
                                    ...
                                    <CardTitle className="text-lg">
                                        Skor Minimal Mandiri
                                    </CardTitle>
                                </div>
                            </CardHeader>
                            <CardContent>
                                <p className="text-3xl font-black text-blue-600 dark:text-blue-400">
                                    {summary.SKOR_MINIMAL}
                                </p>
                                <p className="text-sm text-blue-600/70 dark:text-blue-400/70 mt-1">
                                    Ambang batas untuk mempertahankan status
                                    Desa Mandiri.
                                </p>
                            </CardContent>
                        </Card>
                        ...
                        <Card className="border-none shadow-sm bg-green-500/10">
                            ...
                            <CardContent>
                                <p className="text-3xl font-black text-green-600 dark:text-green-400">
                                    {summary.PENAMBAHAN > 0 ? '+' : ''}
                                    {summary.PENAMBAHAN.toFixed(4)}
                                </p>
                                <p className="text-sm text-green-600/70 dark:text-green-400/70 mt-1">
                                    Perubahan skor dibandingkan dengan periode
                        ...
                        <div className="bg-card p-6 rounded-2xl border border-dashed border-border flex items-start gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-muted-foreground leading-relaxed mt-1">
                                    Indeks Desa Membangun (IDM) meletakkan
                                    prakarsa dan kuatnya modal sosial masyarakat
                                    sebagai dasar dalam proses pembangunan desa
                                    yang berkelanjutan.
                                </p>
                            </div>
                        </div>
            ...
            <section className="px-4 lg:px-12 py-16 bg-muted/50">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {indicatorRows.map((row, idx) => (
                        <Card
                            key={idx}
                            className="border-none shadow-sm hover:shadow-md transition-shadow group overflow-hidden bg-card"
                        >
            ...
            <section className="px-4 lg:px-12 py-12 bg-background flex flex-col items-center text-center">
                <div className="max-w-2xl">
                    ...
                    <h3 className="text-2xl font-bold text-foreground">
                        {idm.IDENTITAS[0].nama_desa}, Kec.{' '}
                        {idm.IDENTITAS[0].nama_kecamatan}
                    </h3>
                    <p className="text-sm text-muted-foreground mt-2">
                        Kabupaten {idm.IDENTITAS[0].nama_kab_kota}, Provinsi{' '}
                        {idm.IDENTITAS[0].nama_provinsi}
                    </p>
