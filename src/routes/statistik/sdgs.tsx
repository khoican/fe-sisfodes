...
export const Route = createFileRoute('/statistik/sdgs')({
  head: () => ({
    meta: [
      ...
      {
        name: 'description',
        content: 'Capaian 18 tujuan pembangunan berkelanjutan (SDGS) Desa Sumberkejayan untuk mewujudkan kemandirian dan kesejahteraan masyarakat.',
      },
    ],
  }),
  ...
  return (
    <main className='w-full'>
      ...
      <section className='bg-white px-4 lg:px-12 py-12 rounded-b-3xl border-b shadow-sm'>
        <div className='flex flex-col lg:flex-row gap-12 items-center'>
          <div className='flex-1'>
            <Badge variant='primary' className='mb-4 uppercase tracking-widest'>Sustainability</Badge>
            <h1 className='text-5xl font-extrabold leading-tight'>
              SDGs <span className='text-primary'>Desa</span>
            </h1>
            <p className='text-gray-600 mt-6 text-lg max-w-2xl'>
              Sustainable Development Goals (SDGs) Desa adalah upaya terpadu untuk mewujudkan 
              desa tanpa kemiskinan, tanpa kelaparan, dan desa ekonomi tumbuh merata.
            </p>
            
            <div className='flex gap-4 mt-8'>
              <div className='px-8 py-4 bg-primary/5 rounded-2xl border border-primary/10 flex flex-col'>
                <span className='text-[10px] font-bold text-primary/70 uppercase tracking-wider'>Skor Rata-rata</span>
                <span className='text-4xl font-black text-primary'>{sdgs.score.average}</span>
              </div>
              <div className='px-8 py-4 bg-gray-50 rounded-2xl border flex flex-col justify-center'>
                <span className='text-[10px] font-bold text-gray-400 uppercase tracking-wider'>Total Tujuan</span>
                <span className='text-2xl font-black text-gray-800'>{sdgs.score.data.length} Goals</span>
              </div>
            </div>
          </div>
          ...
          <div className='hidden lg:flex w-80 h-80 bg-primary/5 rounded-full items-center justify-center p-8 relative overflow-hidden'>
             ...
             <div className='relative z-10 text-center'>
                ...
                <h3 className='text-xl font-bold text-gray-800'>Capaian Global</h3>
                <p className='text-xs text-gray-500 mt-2'>Aksi lokal untuk tujuan global</p>
             </div>
          </div>
        </div>
      </section>
      ...
      <section className='px-4 lg:px-12 py-16'>
        ...
        <div className='h-[400px] w-full bg-white p-6 rounded-3xl shadow-sm border mt-8'>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={sdgs.chart}>
              ...
              <Bar dataKey="score" radius={[4, 4, 0, 0]}>
                {sdgs.chart.map((entry, index) => (
                  <Cell key={`cell-${index}`} fill={getProgressColor(entry.score)} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
      ...
      <section className='px-4 lg:px-12 py-16 bg-gray-50/50'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6'>
          {filteredGoals.map((goal) => (
            <Card key={goal.goals} className='group border-none shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden bg-white flex flex-col'>
              ...
              <CardContent>
                <div className='flex items-center justify-between mb-2'>
                  <span className={`text-xl font-black ${getScoreColor(goal.score)}`}>
                    {goal.score}
                  </span>
                  <span className='text-[10px] font-bold text-gray-400 uppercase'>Capaian</span>
                </div>
                
                <div className='w-full bg-gray-100 rounded-full h-1.5 overflow-hidden'>
                  <div 
                    className={`h-full rounded-full transition-all duration-1000 ${getProgressColor(goal.score)}`} 
        ...
        {filteredGoals.length === 0 && (
          <div className='text-center py-20 bg-white rounded-3xl border border-dashed'>
            <p className='text-gray-400'>Tujuan SDGs yang Anda cari tidak ditemukan.</p>
          </div>
        )}
      </section>
         ...
         <div className='bg-primary rounded-[2rem] p-12 text-white flex flex-col md:flex-row items-center gap-12 relative overflow-hidden'>
            ...
            <div className='flex-1 z-10'>
               ...
               <p className='text-white/80 leading-relaxed max-w-2xl'>
                 Pemerintah Desa Sumberkejayan terus berupaya mengintegrasikan tujuan-tujuan SDGs dalam setiap 
                 kebijakan pembangunan desa. Kami percaya bahwa masa depan yang lebih baik dimulai dari aksi nyata di tingkat desa.
               </p>
            </div>
            <div className='shrink-0 z-10'>
               <div className='bg-white/20 backdrop-blur-md p-6 rounded-2xl border border-white/30'>
                  <p className='text-xs font-bold uppercase tracking-widest mb-2 opacity-80'>Target Rata-rata</p>
                  <p className='text-4xl font-black'>100.00</p>
                  <p className='text-xs mt-2 opacity-60 italic'>Tujuan Akhir 2030</p>
               </div>
            </div>
         </div>
      </section>
