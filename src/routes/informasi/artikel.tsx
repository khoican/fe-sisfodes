...
export const Route = createFileRoute('/informasi/artikel')({
  ...
  loader: async ({ context }) => {
    const artikel = await context.queryClient.ensureQueryData(artikelQueryOptions())
    return {
    ...
    <main className='w-full px-4 lg:px-12 py-8 bg-background'>
      <section className='w-full lg:w-1/2 py-16 flex flex-col gap-6'>
        ...
        <p className='text-muted-foreground text-lg'>
          Berbagi pengetahuan, pengalaman, dan pemikiran untuk membangun masyarakat 
          Desa Sumberkejayan yang lebih cerdas dan informatif.
        </p>
      </section>
          ...
          <section className='w-full grid md:grid-cols-2 gap-6 mt-6'>
            ...
            <div className='w-full flex flex-col gap-8 lg:gap-0'>
              <Suspense fallback={<div className='space-y-4'><CardSkeleton /><CardSkeleton /></div>}>
                {latestArtikel.slice(1, 4).map((item, index) => (
                  <NewsCard
                    key={index}
                    {...item}
                    layout='horizontal'
      ...
      <section className='w-full mt-16'>
        <h2 className='text-2xl font-bold text-foreground'>Semua Artikel</h2>
        <p className='text-muted-foreground mt-2'>
          Jelajahi berbagai topik menarik yang kami sajikan khusus untuk warga Desa Sumberkejayan.
        </p>
        ...
        <div className='grid md:grid-cols-2 lg:grid-cols-3 w-full gap-6 mt-12'>
          <ClientOnly>
            <Suspense fallback={<div className='col-span-full grid grid-cols-1 md:grid-cols-3 gap-6'><CardSkeleton /><CardSkeleton /><CardSkeleton /></div>}>
              {artikelData
                .slice(artikelData.length > 4 ? 4 : 0, artikelData.length)
