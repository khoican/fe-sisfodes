...
const CardSkeleton = () => <div className='w-full h-64 bg-muted animate-pulse rounded-xl' />
...
export const Route = createFileRoute('/informasi/penghargaan')({
  ...
  loader: async ({ context }) => {
    const penghargaan = await context.queryClient.ensureQueryData(penghargaanQueryOptions())
    return {
    ...
    <main className='w-full px-4 lg:px-12 py-8 bg-background'>
      <section className='w-full lg:w-1/2 py-16 flex flex-col gap-6'>
        ...
        <p className='text-muted-foreground text-lg'>
          Rekam jejak prestasi dan penghargaan yang menjadi bukti nyata dedikasi seluruh 
          elemen masyarakat dalam memajukan Desa Sumberkejayan.
        </p>
      </section>
        ...
        <ClientOnly>
          <section className='w-full grid md:grid-cols-2 gap-6 mt-6'>
            <Suspense fallback={<CardSkeleton />}>
              {latestPenghargaan.slice(0, 1).map((item, index) => (
                <NewsCard
                  key={index}
                  {...item}
                  className={{
            ...
            <div className='w-full flex flex-col gap-8 lg:gap-0'>
              <Suspense fallback={<div className='space-y-4'><CardSkeleton /><CardSkeleton /></div>}>
                {latestPenghargaan.slice(1, 4).map((item, index) => (
                  <NewsCard
                    key={index}
                    {...item}
                    layout='horizontal'
      ...
      <section className='w-full mt-16'>
        <h2 className='text-2xl font-bold text-foreground'>Daftar Penghargaan</h2>
        <p className='text-muted-foreground mt-2'>
          Apresiasi yang telah diterima oleh Desa Sumberkejayan dari berbagai instansi dan tingkatan.
        </p>

        <div className='grid md:grid-cols-2 lg:grid-cols-3 w-full gap-6 mt-12'>
          <ClientOnly>
            <Suspense fallback={<div className='col-span-full grid grid-cols-1 md:grid-cols-3 gap-6'><CardSkeleton /><CardSkeleton /><CardSkeleton /></div>}>
              {penghargaanData
                .slice(penghargaanData.length > 4 ? 4 : 0, penghargaanData.length)
