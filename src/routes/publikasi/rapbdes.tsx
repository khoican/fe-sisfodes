...
export const Route = createFileRoute('/publikasi/rapbdes')({
  head: () => ({
    meta: [
      ...
      {
        name: 'description',
        content: 'Rancangan Anggaran Pendapatan dan Belanja Desa (RAPBDes) Sumberkejayan.',
      },
    ],
  }),
  loader: async ({ context }) => {
    const publication = await context.queryClient.ensureQueryData(publicationQueryOptions('rapbdes'))
    return {
      ...
      <section className='bg-background px-4 lg:px-12 py-12 rounded-b-3xl border-b shadow-sm'>
        <div className='max-w-4xl'>
          ...
          <p className='text-muted-foreground mt-6 text-lg'>
            Rancangan Anggaran Pendapatan dan Belanja Desa (RAPBDes) adalah dokumen draf usulan 
            anggaran desa sebelum ditetapkan menjadi APBDes resmi tahunan.
          </p>
        </div>
      </section>
          ...
          <div className='lg:col-span-1 space-y-6'>
            ...
            <div className='bg-blue-500/10 border border-blue-500/20 p-6 rounded-2xl flex gap-4 mt-8'>
              ...
              <p className='text-xs text-blue-700 leading-relaxed'>
                RAPBDes ini sedang dalam tahap pembahasan atau konsultasi publik. Masukan dari warga 
                sangat kami harapkan untuk penyempurnaan anggaran desa.
              </p>
            </div>
          </div>
               ...
               <div className='flex gap-2'>
                  <Button variant='outline' size='sm' asChild>
                    <a href={selectedDoc.file_url} target='_blank' rel='noopener noreferrer'>
                       <Download className='mr-2 h-4 w-4' /> Unduh Rancangan
                    </a>
                  </Button>
               </div>
            </div>
            ...
            <Card className='border-none shadow-xl shadow-muted/10 rounded-3xl overflow-hidden bg-muted min-h-[600px] flex flex-col'>
               ...
               <div className='flex-1 relative bg-muted flex items-center justify-center p-8'>
                  <div className='text-center max-w-sm'>
                     ...
                     <h3 className='font-bold text-lg'>Tinjau RAPBDes</h3>
                     <p className='text-muted-foreground text-sm mt-2 mb-8'>
                        Anda dapat memeriksa draf anggaran melalui pratinjau ini. 
                        Pastikan Anda memiliki aplikasi pembaca PDF untuk kenyamanan terbaik.
                     </p>
                     <Button asChild className='rounded-xl px-8'>
                        <a href={selectedDoc.file_url} target='_blank' rel='noopener noreferrer'>
                           <ExternalLink className='mr-2 h-4 w-4' /> Buka di Jendela Baru
                        </a>
                     </Button>
                  </div>
               </div>
