    ...
    (error: AxiosError<ApiResponse>) => {
        ...
        if (error.response) {
            ...
            switch (status) {
                case 400:
                    errorMessage =
                        serverMessage || 'Permintaan tidak valid (Bad Request).'
                    break
                case 401:
                    errorMessage =
                        serverMessage || 'Sesi berakhir, silakan login kembali.'
                ...
                case 404:
                    errorMessage =
                        serverMessage || 'Data atau endpoint tidak ditemukan.'
                    break
                case 422:
                    errorMessage =
                        serverMessage ||
                        'Gagal memproses data (Validation Error).'
                    break
                case 429:
                    errorMessage =
                ...
                case 503:
                    errorMessage =
                        'Layanan sedang tidak tersedia (Maintenance).'
                    break
                default:
